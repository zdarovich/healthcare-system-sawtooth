...
func main() {
	...
	filePath := "test/report/2021-11-26T13:40:42-Test_1_User_Creates_Concurently_Shares_Data_Up_To_100000_bytes.csv"

	_, ytps1, ylat1, ymem1 := GetData(filePath)

	filePath = "test/report/2021-11-26T14:22:00-Test_3_User_Creates_Concurently_Shares_Data_Up_To_100000_bytes.csv"

	x2, ytps3, ylat3, ymem3 := GetData(filePath)

	filePath = "test/report/2021-11-26T14:06:51-Test_5_User_Creates_Concurently_Shares_Data_Up_To_100000_bytes.csv"
	...
	f2, _ := os.Create("resources/charts/1_3_5_share_data_chart_bar.html")
...
func GetData(filePath string) ([]int, []float64, []float64, []int) {
	...
	for i := 0; i < len(records); i += binNum {
		...
		sr := getAverageInt(xAxisBin) / 1000       // bytes to kb
		tps := getAverageFloat64(yAxisBinTps) * 10 // normalize tps
		lat := getAverageFloat64(yAxisBinLatency)
		mem := getAverageInt(yAxisBinMemory) / 1000 // bytes to kb
...
func Get1510ClientThroughputSendRatechart(xAxis1 []int, yAxisData1, yAxisData5, yAxisData10 []float64) *charts.Bar {
	...
	bar.SetGlobalOptions(
		...
		charts.WithTitleOpts(opts.Title{
			Title: "Data sharing throughput performance",
		}),
...
func Get1510ClientLatencySendRateLine(xAxis1 []int, yAxisData1, yAxisData5, yAxisData10 []float64) *charts.Line {
	...
	line.SetGlobalOptions(
		...
		charts.WithTitleOpts(opts.Title{
			Title: "Data sharing latency performance",
		}),
		charts.WithXAxisOpts(opts.XAxis{
			Name: "Send rate(kb/seconds)",
			SplitLine: &opts.SplitLine{
...
func Get1510ClientMemorySendRatechart(xAxis1 []int, yAxisData1, yAxisData5, yAxisData10 []int) *charts.Bar {
	...
	bar.SetGlobalOptions(
		...
		charts.WithTitleOpts(opts.Title{
			Title: "Data sharing memory performance",
		}),
		charts.WithInitializationOpts(opts.Initialization{
			Width:  "3300px",
			Height: "600px",
